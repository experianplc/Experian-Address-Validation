...
import { AddressValidationMode, defaults } from './search-options';
import { datasetCodes } from './datasets-codes';
import { translations } from './translations';
import { Picklist } from './class-types';
var AddressValidation = /** @class */ (function () {
    function AddressValidation(options) {
        ...
        this.lookupEndpoint = 'address/lookup/v2';
        ...
        this.what3WordCountries = ['GBR'];
        this.what3WordsKeyword = 'what3words';
        this.poweredByLogo = {
            element: null,
        ...
        this.result = {
            ...
            show: function (data) {
                ...
                if (_this.searchType === AddressValidationMode.AUTOCOMPLETE || (data.result.address && data.result.confidence !== 'No matches')) {
                    ...
                    for (var i = 0; i < Object.keys(data.result.address).length; i++) {
                        var key = Object.keys(data.result.address)[i];
                        var addressComponent = data.result.address[key];
                    ...
                    if (_this.searchType === AddressValidationMode.AUTOCOMPLETE) {
            ...
            createFormattedAddressContainer: function () {
                ...
                var position = _this.searchType === AddressValidationMode.SINGLELINE ? _this.inputs.length - 1 : 0;
    ...
    AddressValidation.prototype.getEnrichmentData = function (globalAddressKey) {
        if (globalAddressKey) {
            var data = {
    ...
    AddressValidation.prototype.handleEnrichmentResult = function (response) {
        ...
        if (this.currentCountryCode) {
            ...
            if (this.currentDataSet) {
                ...
                if (this.searchType === AddressValidationMode.AUTOCOMPLETE) {
                    setTimeout(function () { return _this.handlePromptsetResult({ result: { lines: [{ example: _this.options.placeholderText, prompt: 'Address', suggested_input_length: 160 }] } }); });
                    return;
                }
    ...
    AddressValidation.prototype.lookupDatasetCode = function () {
        var _this = this;
        var item = datasetCodes.find(function (dataset) { return dataset.iso3Code === _this.currentCountryCode && dataset.searchType.includes(_this.searchType); });
        if (item) {
    ...
    AddressValidation.prototype.registerInputs = function (inputs) {
        ...
        this.inputs.forEach(function (input) {
            ...
            if (_this.searchType === AddressValidationMode.AUTOCOMPLETE) {
                ...
                input.setAttribute('placeholder', _this.options.placeholderText);
            }
    ...
    AddressValidation.prototype.generateLookupDataForApiCall = function (input, shouldGetSuggestions) {
        // If a dataset code hasn't been set yet, try and look it up
        if (!this.currentDataSet) {
            this.currentDataSet = this.lookupDatasetCode();
        }
    ...
    AddressValidation.prototype.search = function (event) {
        event.preventDefault();
        this.currentSearchTerm = this.inputs.map(function (input) { return input.value; }).join(',');
        ...
        if (this.canSearch()) {
            ...
            var data = this.isWhat3Words ? this.generateLookupDataForApiCall(this.currentSearchTerm, true) : this.generateSearchDataForApiCall();
            ...
            var url = void 0, headers = void 0, callback = void 0;
    ...
    AddressValidation.prototype.createPicklist = function () {
        ...
        this.picklist.show = function (items) {
            ...
            if (((_a = _this.picklist.items) === null || _a === void 0 ? void 0 : _a.length) > 0) {
                ...
                if (_this.searchType === AddressValidationMode.VALIDATE) {
        ...
        this.picklist.showLookup = function (items) {
            ...
            _this.isWhat3Words = false;
            // Store the picklist items
            _this.picklist.lookupItems = items === null || items === void 0 ? void 0 : items.result.addresses;
            _this.picklist.handleCommonShowPicklistLogic();
            if (((_a = _this.picklist.lookupItems) === null || _a === void 0 ? void 0 : _a.length) > 0) {
                // Iterate over and show results
                _this.picklist.lookupItems.forEach(function (item) {
                    // Create a new item/row in the picklist
                    var listItem = _this.picklist.createLookupListItem(item);
                    _this.picklist.list.appendChild(listItem);
                    // Listen for selection on this item
                    _this.picklist.listen(listItem);
                });
        ...
        this.picklist.createListItem = function (item) {
            ...
            row.innerHTML = _this.picklist.addMatchingEmphasis(item);
        ...
        this.picklist.createWhat3WordsListItem = function (item) {
            ...
            row.className = _this.what3WordsKeyword;
        ...
        this.picklist.refine = {
            ...
            isNeeded: function (response) {
                return _this.searchType !== AddressValidationMode.AUTOCOMPLETE && (response.result.confidence === 'Premises partial' || response.result.confidence === 'Street partial' || response.result.confidence === 'Multiple matches');
            },
            ...
            enter: function (event) {
                ...
                if ((event instanceof KeyboardEvent && event.key === 'Enter') || event instanceof MouseEvent) {
                    ...
                    if (_this.picklist.refine.element.value) {
                        ...
                        _this.request.send("".concat(_this.baseUrl).concat(_this.refineEndpoint, "/").concat(key), 'POST', _this.result.handleValidateResponse, data);
                    }
                }
        ...
        this.picklist.listen = function (row) {
            row.addEventListener('click', _this.picklist.pick.bind(null, row));
        };
    ...
    AddressValidation.prototype.format = function (url) {
        // Trigger an event
    ...
    AddressValidation.prototype.refine = function (key) {
        ...
        this.request.send("".concat(this.baseUrl).concat(this.stepInEndpoint, "/").concat(key), 'GET', this.picklist.show);
    };
    AddressValidation.prototype.lookup = function (key) {
        ...
        var lookupV2Request = this.generateLookupDataForApiCall(key, false);
        var url = this.baseUrl + this.lookupEndpoint;
